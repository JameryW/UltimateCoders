...
export function useDashboard() {
  ...
  return {
    health, workers, tasks, scheduler, circuitBreaker, eventLog, interactionLog,
    connected, setConnected, handleSnapshot, handleTaskEvent,
    mergeGrpcTasks, mergeGrpcHealth, fetchInitial,
  };
}
...
function mergeSubtaskEvent(subtasks: SubtaskSummary[], ev: TaskEvent): SubtaskSummary[] {
  ...
  const statusMap: Record<string, string> = {
    subtask_assigned: "assigned", subtask_started: "in_progress",
    subtask_completed: "completed", subtask_failed: "failed",
  };
  const newStatus = statusMap[ev.type];
  if (!newStatus) return subtasks;
  const existing = subtasks.find((s) => s.id === sid);
  if (existing) return subtasks.map((s) => s.id === sid ? { ...s, status: newStatus } : s);
  return [...subtasks, { id: sid, description: String(ev.data.description ?? ""), status: newStatus, depends_on: (ev.data.depends_on as string[]) ?? [] }];
}
