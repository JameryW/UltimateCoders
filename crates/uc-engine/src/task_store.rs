...
impl TaskStore {
    ...
    /// Submit a new task: store it and return.
    ///
    /// Insert-only (T5 #641): the Rust-side newline-split decomposition fork is
    /// gone — decomposition belongs to the TS planner / Python orchestrator
    /// (D2). NATS availability must never change WHAT.
