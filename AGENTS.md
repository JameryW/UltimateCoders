...
GitHub Issues are preferred, with a local Markdown fallback when `gh` is unavailable or unauthenticated. See `docs/agents/issue-tracker.md`.

### Triage labels

Use the canonical state and category mapping in `docs/agents/triage-labels.md`.

### Domain docs

This is a single-context repository. See `docs/agents/domain.md` for the glossary and ADR reading rules.

## Codex Issue Workflow

For a new product capability, a multi-ticket initiative, or a named tracker ticket, load `$ultimatecoders-issue-flow`. It owns the Matt-skills-to-Trellis handoff, tracker fallback, isolated-task rule, and quality gates. See `docs/workflows/codex-issue-flow.md` for the human quick start.
