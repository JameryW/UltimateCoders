...
**§六 的覆盖边界（同一审计显式写出）**：本节是**迁移程序**（P0/P1/P2）的执行状态，收录 **T1–T20**；**T21–T29 属框架卫生线**（spec 引用卫生 #672/#673/#675、journal 账本 #674/#676、T24 子句护栏 #677），与本件范围无关，其记录在各自 issue 与 archived task 目录，**本件不重复**。⚠️ 写出来的理由：若按「T 编号连续」推断，会把这九张票当成**未交付的迁移票** —— 实测它们**全部 `completed` 并已归档**、对应 issue 全部关闭，且 `task.json` 的 `map` 字段为 `None`（**T16–T19 才是 `map = 656`**，即真正挂在本程序地图上的票）。
