...
`Jamery Wang` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Jamery Wang/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Jamery Wang/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee Jamery Wang` to
...
python ./.trellis/scripts/task.py archive 00-join-jamery-wang
```
