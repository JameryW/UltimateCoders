...
* [x] Memory bridge 支持所有 3 层 scope (short_term→task, long_term→global, metadata→project)，正确路由到 gRPC
...
* [ ] ~~读写子任务动态容量：无 file_constraints → 2x 并发，多 file_constraints → 限制~~ **deferred** — 当前瓶颈在 LLM API rate limit，多 worker + 资源监控后再加
* [ ] ~~大量并行任务不 OOM（signal + queue 架构）~~ **deferred** — subprocess Worker 已有基本 OOM 防护，需真实内存压力测试验证

### Phase 4: IRC 子 agent 间通信

* [x] 子 agent 间共享上下文通过 uc_memory tool（替代 IRC, worker prompt 已引导）
...
* [x] 子 agent 可通过 uc_memory tool 主动查询完整细节 (prompt 含 read/search 示例)
...
* [x] 审查结果写入 uc_memory (subtask_review_<id>) + Dashboard 可见 (syncTaskToGrpc)
...
| omp 子 agent 资源消耗 | 大量并行任务 OOM | subprocess 隔离 + 信号量限并发（deferred: signal+queue 动态容量） |
...
|------|-----------|----------------|----------|
| 失败重试 | max_retries + backoff | ✅ executeSubtaskWithRetry (maxRetries=2, exponential backoff) | maxRetries=2, exponential backoff |
| Checkpoint | save/load modified_files/tool_calls/error | ✅ saveCheckpoint/loadCheckpoint + gRPC sync | 同 schema |
| Resume | load checkpoint skip completed | ✅ resumeFromWave persisted + checkpoint restore | 对齐 checkpoint 字段 + resume 逻辑 |
| 并发控制 | _dynamic_capacity (read 2x, write limit) | **deferred** — 多 worker 后再加 | 同策略 |
