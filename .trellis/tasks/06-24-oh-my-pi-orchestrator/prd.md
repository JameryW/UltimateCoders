...
* [ ] `memory-bridge.ts` 独立文件抽取 — uc_memory/uc_search 从 extension.ts 内联代码重构为独立模块
* [ ] Memory bridge 支持所有 3 层 scope (short_term/long_term/metadata)，正确路由到 gRPC

### Phase 4: 错误恢复 + checkpoint/resume

* [ ] 子任务失败自动重试 (maxRetries=2, exponential backoff 5s→10s→20s)
* [ ] `_save_checkpoint` 保存 modified_files/tool_calls/error 到 TaskStore
* [ ] `_load_checkpoint` 恢复完整 SubtaskResult（同 Python 侧 schema）
* [ ] `/uc resume <task-id>` 从 checkpoint 断点继续，跳过已完成子任务
* [ ] 重试次数超限 → 标记子任务 failed + 通知 Dashboard

### Phase 4: 并发控制信号量

* [ ] 信号量限制并行子 agent 数量（默认 maxConcurrency=3）
...
* [ ] 完成的子 agent 结果自动写入 uc_memory（scope: task, key: subtask_result_<id>）
* [ ] 后续子 agent prompt 自动注入已完成依赖子任务摘要（500字截断）
* [ ] 子 agent 可通过 uc_memory tool 主动查询完整细节

### Phase 4: Advisor 集成

* [ ] Supervisor agent 做子任务结果审查（approved/issues/suggestions）
* [ ] 审查超时 (60s) → 自动批准（避免阻塞）
* [ ] 审查拒绝 → 子任务标记 failed + 可重试
