...
## Session 24: T21: .trellis/spec 的 文件:行号 引用普查与口径 —— 定「符号优先」并落地结构性回归守卫

**Date**: 2026-09-16
**Task**: T21: .trellis/spec 的 文件:行号 引用普查与口径 —— 定「符号优先」并落地结构性回归守卫
**Branch**: `main`

### Summary

对 .trellis/spec/** 的 148 处 文件:行号 引用做全量普查，得出决定性否证：0/148 越界 ⇒ 过期引用的形态是「行号合法、内容错误」，结构性检查（存在性+范围）对其价值≈0。新增 scripts/check-spec-refs.py 做三层判定（结构性 fail-closed / 歧义 / 符号漂移），用 T20 的手工真值 6/6 交叉校验，消融 5/5（含阴性对照与优先级回归）。修掉 2 处结构缺陷使守卫落地即绿；口径定为「符号引用优先」，148 处重写与内容匹配校验器留给后续票。

### Main Changes

## 为什么会有这一票：T20 自己派生的

T20 收口时把「规范里的 `文件:行号` 引用系统性过期」记成了 **#672**：偏移 **+208 ~ +391 不等**（`_emit_step_event` +312 / `_run_single_step` +374 / `_render_step_prompt` +391 / `_execute_in_sandbox` +208）⇒ **无法机械 +N 修正**。票面写得很清楚：**第一步应是普查并出清单，而不是直接改**；且「本票先不开实现，先定口径」。

本会话即按此执行：**普查 + 定口径 + 顺带把确定性的那一类修掉并用守卫钉住**。

## 一手普查：148 处 / 12 个规范 / 33 个目标文件

不再抽样，改为**写一个可复算的普查工具**（`scripts/check-spec-refs.py`，纯 stdlib），一次性导出全量：
...
| 分布在 | **12** / 31 个规范文件 |
| 指向的目标文件 | **33** |
| 单文件最多 | `agent-capability-spec.md`（78 处） |

## 🔴 本轮最值钱的一条：**0 / 148 越界**

这个数字直接**否掉了 issue 里候选方案 2 的弱版本**（存在性 + 范围检查）。本仓的过期引用**几乎全是「行号合法、内容错误」**，所以：

**结构性检查对「过期」本身的价值 ≈ 0。**

它唯一能抓到的是**另一类**缺陷 —— 死文件、目录前缀写错、行号越界。那 2 处确实值得抓，但**不是本票的起因**。这一条如果不实测，很容易写出「加个校验器就好了」这种听起来对、实际没用的方案。

## 结构缺陷 2 处（确定、零假阳性）

| 类型 | 位置 | 事实 |
|---|---|---|
| `PATH_FORM` | `agent-capability-spec.md:720` 写 `uc-engine/src/events.rs` | 真实路径是 **`crates/uc-engine/src/events.rs`**（漏目录前缀） |
| `MISSING_FILE` | `logging-guidelines.md:75` 引 `/agent/rate_limiter.py:16` | 死于 **`31ca4d2`「remove dead Python rate_limiter.py (#560)」** |

第二处用 `git log --diff-filter=D` 确认是**刻意删除的死码** ⇒ 应删引用，而不是改指向。

## 符号锚定复核：11 处大幅漂移，+119 ~ +391

按「同一规范行点名的符号，其真实 `def` 行」比对（即 T20 手工取证的方法扩到全量）：11 处偏移**互不相同**，集中在 `worker.py`（`_emit_step_event` 1347 / `_execute_steps` 1365 / `_run_single_step` 1620 / `_output_to_json` 1785 / `_render_step_prompt` 1817 / `_execute_in_sandbox` 1202）与 `sandbox.py`（`_parse_agent_file_changes` 1324）⇒ 与 issue 判断一致。

另有 8 处偏移很小（−27 ~ +11）—— **不足以判定过期**，可能本来就有意指向定义上方的文档注释或属性块（`EngineApi` 定义在 `engine.rs:30` 而引用写 `:25`）。工具把它们列为 advisory，**不判红**。

## 工具的两处自我更正（本票的方法论副产品）

**第一处：第一版守卫报了 63 处「失败」，其中 61 处是假阳性。** 我把**裸文件名**（`worker.py`）当成「目录写错」，但那是本仓 spec 的**成文写法**（规范自身已声明目标模块）⇒ 唯一命中应判 OK。若只看「它抓到了 63 个」就去改，等于拿噪声改掉 61 处无罪的引用。

**第二处更隐蔽：符号复核会把结构性命中覆盖掉。** `uc-engine/src/events.rs`（前缀写错）一开始被判成 advisory 的 `STALE`，于是**真正的结构缺陷被降级、从报告里消失** —— 「两条规则写同一个字段」的经典优先级 bug。已改为**结构优先、advisory 永不掩盖结构**，并专设 M4 突变把这条钉住。

## 守卫的诚实定位：**回归守卫，不是过期探测器**

因为 0/148 越界这个事实，守卫**绿 ≠ 引用没过期**。这一点**明写进脚本 docstring**，避免下一个人把它当证据 —— 那正是本仓最贵的「间接信号当事实」。工具另有两条局限已记账：**104/148 处引用在同一行上没有可判定符号**（判不了 ⇒ 计数是**下界**），以及**引代码而不点名符号的引用完全在盲区**。

## 一个手工微型样本，正好补上工具的盲区

`logging-guidelines.md` 的「Real examples」列表 6 条里 **5 条行号已漂、1 条文件已删**，而**全部通过范围检查**：`worker.py:51`→**55**、`llm.py:16`→**18**、`conflict.py:20`→**19**、`sandbox.py:15`→**25**。这些行**不点名符号** ⇒ 符号锚定也抓不到，**只有内容匹配**（第 N 行是否含 `logging.getLogger`）才会红 —— 这就是口径里「强版本校验器」的价值来源。

## 口径（本票的决策）

逐条验收 issue 的三方案：

- **① 符号引用优先 ⇒ 采纳为本体。** 符号名不因上方插入而漂移；本票 11 处大幅漂移**全部**是符号引用，**去掉行号即一次全消**。
- **② 保留行号 + 校验器 ⇒ 只采纳其确定性部分。** 弱版本抓不到过期（0/148 越界）；强版本（内容匹配）确有价值，但需先把引用改写成可匹配的形状 ⇒ 随重写一起做。
- **③ 标注基线 ⇒ 不采纳为本体。** 它把负担推给人，而漂移是**持续**的（T19/T20 都在动这些文件）；仅作既无符号、又无内容锚时的兜底。

## 消融与交叉校验（判据不是「我说它对」）

- **消融 5/5 PASS**：一次一处突变 → 跑守卫 → 断言**探针行**的判定与结构失败数的**增量** → 按字节恢复并校 sha256。M0 阴性对照（合法引用）结构失败数 2→2 **不变**；M1 死文件 → `MISSING_FILE`；M2 目录前缀错 → `PATH_FORM`；M3 行号越界 → `OUT_OF_RANGE`；**M4 同行含符号 + 坏路径 → `PATH_FORM`（不是 advisory 的 `STALE`）**。
- **地面真值 6/6 一致**：工具输出与 T20 手工测得的定义行**逐个吻合**（1347 / 1365 / 1620 / 1785 / 1817 / 1202）。这是「工具可被信任、而不是又一个自写的间接信号」的前提。
- ruff `All checks passed!`；改动是**小改**（numstat `1/1` + `4/5`），不是整文件重写。

## 交付边界（有意不做的事）

- **不重写**那 148 处引用的符号优先形态 —— 票面写明「先定口径」，重写留给后续票。
- **不接 CI**：`scripts/**` 与 `.trellis/**` **均不在任何 workflow 的 `paths` 内** ⇒ 本提交**实测零 CI 触发**（已用 `gh run list` 核过：最新 run 仍是 `6d7653c`）。守卫目前是**手动**的；接 CI 会改变触发面（`.trellis/**` 一改就跑 Python CI），属独立的 gate 决策。
...
| Hash | Message |
|------|---------|
| `d6dbba4` | `docs(spec): audit every path:line ref under .trellis/spec and add a structural guard (T21 #672)` |

### Testing

- **守卫本体**：`python scripts/check-spec-refs.py` → `0 structural failure(s)`，exit 0（修掉 2 处结构缺陷之后）。
- **消融 5/5 PASS**（`python .scratch/t672-ablation.py`）：M0 阴性对照（合法引用）结构失败数 **2→2 不变**；
  M1 死文件 → `MISSING_FILE`；M2 目录前缀错 → `PATH_FORM`；M3 行号越界 → `OUT_OF_RANGE`；
  **M4 同行含符号 + 坏路径 → `PATH_FORM`（不是 advisory 的 `STALE`）**。每轮结束**按字节恢复**并 `restored byte-identically`。
- **地面真值 6/6**：工具输出与 T20 手工测得的定义行逐个吻合（`_emit_step_event` 1347 / `_execute_steps` 1365 /
  `_run_single_step` 1620 / `_output_to_json` 1785 / `_render_step_prompt` 1817 / `_execute_in_sandbox` 1202）。
- **行号修正逐条回读**：`logging-guidelines.md` 的 4 个新行号**写前**均回读确认目标行含 `logging.getLogger`。
- **ruff**：`All checks passed!`（venv，line-length 100，select E/F/I/N/W/UP）。
- **改动范围**：`git diff --numstat` = `1/1` + `4/5` ⇒ 小改，非整文件重写。
- ⚠️ **本提交实测零 CI 触发** —— `gh run list` 核过：最新 run 仍是 `6d7653c`。原因：`scripts/**` 与
  `.trellis/**` **均不在任何 workflow 的 `paths` 内**（一手读 `.github/workflows/*.yml`）⇒ 守卫目前**是手动的**。

### Status

[OK] **Completed** — T21 #672 已交付并归档（`d6dbba4`）。**不挂 P2 地图**：本票是仓级文档卫生，
与地图开放决策无关（P2-1/P2-2/P2-3 的本体仍需外部「方案第 21 节」原文）。

### Next Steps

- **#673**（本票派生，已开）：按口径重写 148 处引用为「符号优先」；为「引代码」型引用加**内容匹配**校验器；
  并决定是否把守卫接进 CI（⚠️ `.trellis/**` 若进 `paths`，每次 trellis 改动都会触发 Python CI ⇒ 独立的 gate 决策）。
- **P2 本体**：仍卡在外部「方案第 21 节」原文，不臆造。
- 待沉淀（技能）：① **先验检查器的 FAIL 真伪** —— 第一版守卫报的 63 处失败里 **61 处是假阳性**；
  ② **两条规则写同一字段 ⇒ 结构结论会被 advisory 覆盖**（已用 M4 钉住）。


## Session 25: T22: 给 spec 引用守卫补「bold/heading 符号」与「内容锚」—— 量出 #673 重写的可行边界

**Date**: 2026-09-16
**Task**: T22: 给 spec 引用守卫补「bold/heading 符号」与「内容锚」—— 量出 #673 重写的可行边界
**Branch**: `main`

### Summary

给 T21 的 spec 引用守卫补两处能力：_symbols_on 此前只在行内代码里认符号（**bold** 与 ### 标题一律判「本行无符号」，实测漏掉 13 条真锚），并新增内容锚（本行被引代码是否出现在目标文件里）。内容锚落地前发现 naive 串匹配 17 处「失败」里 13 处是假阳性（限定名不会原样出现），补归一化后零误报；且刻意做成独立标志而非新 verdict，避免两条同级 advisory 共用一个字段互相掩盖（T21 同型 bug）。消融 10/10，含 M6/M7 双跑证明是新代码起的作用。量出重写工作清单：84/148 今天可去行号，放宽到段落可到 105（刻意不实现），余 43 待兜底或消歧。本票是 #673 的切片 A，#673 保持开放。

### Main Changes

## 起点：T21 的守卫看不见它自己要服务的那个口径

T21 定下「符号优先」口径、并落地了 `scripts/check-spec-refs.py`。但 **#673 的重写只能靠两种锚**：
行号一去掉，**符号名**与**被引代码**就是仅剩的证据 —— 而当时的守卫：

- 只在**行内代码**（反引号）里认符号 ⇒ `**Task**`、`### Task Properties` 一律被判成「本行没有符号」；
- **完全没有内容锚**概念 ⇒ 「本行引的代码在目标文件里到底有没有」无从判起。

⇒ 本票先把仪器补到能看见全部锚，再**量出重写的可行边界**。这是 #673 的**切片 A**；它的本体（重写 148 处）**本票不做**。

## 🔴 内容锚落地前先把它的 FAIL 验真：第一版 17 处「失败」里 13 处是假阳性
...
```
Worker._execute_steps    -> worker.py    实际写的是 def _execute_steps（限定名不会原样出现在文件里）
Worker._run_single_step  -> worker.py    同上
Worker._emit_step_event  -> worker.py    同上
```

⇒ 若照第一版落地，就会一次引入 17 处噪声，其中 **13 处要去改根本没坏的东西** ——
与 T21 那次「63 处失败里 61 处是假阳性」是**同一条教训**，只不过这次在**落笔前**就被「读输出」拦住了。

补「限定名回落成末段 / 去掉 `=True` 尾巴」后只剩 **4** 处不匹配，全是 `step_condition.py` 语法表的
**元变量**（`!expr` / `a && b` / `a || b` / `(expr)`）—— **不是该文件里的代码**。
⇒ 取「被引串须含 `_` `=` `.` `"` 之一」把它们**按构造豁免**，当前语料上内容锚 **0 误报**。

## 🔴 第二条自我更正：我差点又造一个「两条规则写同一个字段」

内容锚的第一版设计是**一个新的 verdict**，且排在 `STALE` **之后**。
这意味着同一条引用**既符号漂移、又内容不匹配**时，后者被前者**掩盖** ——
与 T21 抓到的优先级 bug **同型**，只是层级从「advisory 掩盖 structural」变成「**advisory 掩盖 advisory**」。

⇒ 改为**独立标志** `content_ok`，**不进 verdict 链**；`--json` 同时暴露 `content` / `content_ok` /
`content_candidate_count`。这一步顺带让重写那一刀可以**完全由仪器驱动**，不必再手写普查脚本。

## 更宽的符号检测：STALE 19 → 27，**无一条消失**

`_symbols_on` 改为同时扫行内代码 + `**bold**` + `### 标题`。新增的 8 条**逐条核对**：

**7 条是真锚** —— `**Task**` ×2 / `**OrchestratorConfig**` / `**SearchResultItem**` /
...
⇒ 精度 **7/8**。那 1 条噪声**如实记账**，且**不**把它加进 `SYMBOL_STOPWORDS`
（`delete` 完全可能是真实方法名，为一条噪声收窄词表不划算）。

## 消融 10/10 —— 其中两条是**双跑**，用来钉住「是新代码起的作用」

判据三层：① 探针行的 verdict（走**真 CLI 子进程**）；② structural 计数的 **delta**；
③ 对新能力**双跑**：同一探针再用**改动前**的 `_symbols_on` 收一次，断言**旧检测给出不同答案**。

**为什么必须双跑**：只断言「新检测报 STALE」**证明不了是放宽起的作用** ——
行内代码里的符号也可能解释它。把「旧检测报 `OK`」也钉住，才排除了这个替代解释。
这是本票对**消融方法本身**的补充：**新能力必须证明「没有新代码就不会触发」**。

**一处过程记录**：M7 第一次 **FAIL**，但**错在探针不在代码** ——
我写的探针是 `Probe: ### SubtaskStatus …`，而 `HEADING_RE` 是**行首锚定**的（真实标题都从第 0 列开始）。
改成行首后即 PASS；这反而证明它对真实语料有效（下文那 3 条 `### 标题` 锚确实被抓到）。
...
⇒ **今天就能去行号的 = 84 / 148**。64 条里 **8 条**是裸文件名歧义（`types.py` 命中多个文件），
**56 条**本行没有可匹配的被引代码。

**若把内容窗口从「本行」放宽到「所在段落」**：64 条里 **21 条**能在段落内找到可匹配代码
⇒ 可去行号 **105 / 148**，余 **35 真无锚 + 8 歧义 = 43**。

⚠️ **本票刻意不实现这个放宽**：它把「自证」从**一行**松到**一段**，属**语义放宽**
（候选变多 ⇒ 漏报更少，但误报风险上升），需要它自己的消融证据。故留作下一刀的**建议**，并写进 #673。

最能说明它价值的是 `logging-guidelines.md` 那 10 条：它们引的 `logger = logging.getLogger(__name__)`
就在**上一行**的代码块里，本行只有一个 `path:line`。

## 交付边界（有意不做的事）

- **不做 148 处重写** —— #673 的本体，留给下一刀。
- **不实现段落级内容锚** —— 语义放宽，需独立消融。
- **不接 CI** —— `scripts/**` 与 `.trellis/**` 均不在任何 workflow 的 `paths` 内。
- **不改任何产品代码、不改任何 spec 正文** —— 消融探针按字节恢复，`git status` 只有一处 `M`。

### Git Commits

| Hash | Message |
|------|---------|
| `7475952` | `feat(scripts): teach the spec-ref guard to see bold/heading symbols and quoted content (T22 #673)` |


### Testing

- **守卫本体**：`python scripts/check-spec-refs.py` → `113 ok / 27 stale(advisory) / 8 ambiguous(advisory) / 0 structural`，**exit 0**。
- **消融 10/10 PASS**（`python .scratch/t22-ablation.py`）：
  M0 对照（合法引用 + 已存在的被引代码）structural `0→0`、`content_ok=True`；
  M1 死文件 → `MISSING_FILE`；M2 目录前缀错 → `PATH_FORM`；M3 行号越界 → `OUT_OF_RANGE`；
  **M4 老优先级回归**（符号 + 坏路径）仍判 `PATH_FORM`（不被 advisory 掩盖）；
  M5 内容不匹配 → `content_ok=False`；M5b 内容对照 → `content_ok=True`；M8 本行无被引代码 → 不适用（`content_ok=True`）；
  **M6 粗体符号 / M7 标题符号 → `STALE`，且同一条探针用旧检测跑得到 `OK`（双跑）**。
  每轮结束 `restored byte-identically`（sha256 `4408d2850bea…` 前后一致），并复查 148 条引用数未漂移。
- **锚点分布**直接由 `--json` 的行字段导出（`symbol` / `content_candidate_count`），不是启发式：**84 可去行号 / 64 无锚**。
- **散文噪声如实记账**：新增 8 条 STALE 中 1 条是 `delete` 的散文误配 ⇒ 精度 **7/8**。
- **ruff**：`All checks passed!`（venv）。
- **改动范围**：`git diff --numstat` = `108/5`；文件工作树**统一 CRLF**（CR==LF=398）；`git status` 只有此一处 `M`
  ⇒ 小改，且 **0 处 spec 正文改动**（消融探针按字节恢复）。
- ⚠️ **零 CI 触发已用 `gh run list` 实测**（不从 `paths` 配置推断）：`scripts/**` 与 `.trellis/**` 均不在任何 workflow 的 `paths` 内。

### Status

[OK] **Completed** — 本票交付 `7475952`。**但 #673 保持开放**：本票是它的**切片 A（仪器）**，
本体（按符号优先重写那 148 处引用）**尚未做**；工作清单（84 / 21 / 43 三段）已写进 #673 评论。

### Next Steps

- **#673 切片 B（重写）**：按符号优先重写 148 处引用 —— **84 处今天可直接去行号**；
  **21 处**若把内容锚放宽到「段落」也可去（该放宽是**语义放宽**，须先补它自己的消融）；
  余 **35 处真无锚 + 8 处裸名歧义** ⇒ 前者需基线兜底或人工补锚，后者需先把 `types.py` 消歧。
- **#673 切片 C（CI 决策）**：是否把守卫接进 CI。⚠️ `.trellis/**` 一旦进某个 workflow 的 `paths`，
  每次 trellis 改动都会触发 Python CI ⇒ 属**独立**的 gate 决策。
- **P2 本体**：仍卡在外部「方案第 21 节」原文，不臆造。
