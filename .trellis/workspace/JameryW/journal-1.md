...
## Session 34: 优化任务执行链路：删LLM模式、精简检索、事件去重、双写合并

**Date**: 2026-06-21
**Task**: 优化任务执行链路：删LLM模式、精简检索、事件去重、双写合并
**Branch**: `feat/dashboard-file-browser`

### Summary

删除Worker LLM tool-calling模式，只保留sandbox执行(worker.py 1979→220行)。Orchestrator删除llm_client/rate_limiter/circuit_breaker，精简分解检索(不再预注入代码片段)。NATS消息加message_id，Rust TaskStore HashMap去重+TUI Set幂等。统一NATS事件管道，TaskEventEmitter退化为ring buffer。净删除3642行代码。
