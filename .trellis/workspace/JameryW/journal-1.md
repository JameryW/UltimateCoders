...
实现9项TUI优化: 搜索高亮闭环(修输入bug+in-text高亮), Subtask行内状态条, 工具事件折叠改进, 消息时间间隔标记(修parseHHMM bug), 关键事件通知(StatusBar 3s闪烁), 多Worker负载柱状图, 输入历史搜索Ctrl+G, 长消息行数上限50行, 多窗格布局(≥120col并排)。全部393测试通过。PR #116
