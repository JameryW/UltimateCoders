...
- **EventStore backend is configurable** (`UC_EVENT_BACKEND`, #579). Default `InMemoryEventStore` (lost on restart); `UC_EVENT_BACKEND=nats` (+`UC_NATS_URL`) activates `NatsEventStore` (JetStream, durable — survives restart, checkpoint recover reads the persisted stream). The `NatsEventStore` AGENT_EVENTS stream filter is `>` (captures all subjects — `task.{id}` + `agent.events.{id}`); the old `agent.events.>` filter silently dropped TaskStore events. `NatsEventStore::latest_offset` returns the GLOBAL stream sequence (ignores subject) — accepted because `apply_event_to_snapshot` is idempotent (over-replay is safe).
