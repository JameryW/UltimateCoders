...
> **WatchTask recovery**. The stream still uses a bounded exponential backoff (8 sleeps, ~91s cumulative) for fast transient recovery. When that sequence is exhausted — or the bridge is found disconnected during a retry — the orchestrator resets the bounded counter and schedules a single 30s recovery cycle. The cycle reopens an already-connected stream directly; otherwise it calls the bridge health RPC, which owns connection-level recovery, and retries the cycle while unavailable. The timer is generation-guarded and cancelled during orchestrator destruction, so long gateway outages recover without a restart and stale callbacks cannot revive a closed stream.
