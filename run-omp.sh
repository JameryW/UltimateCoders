...
# ponytail: OMP workspace root = uc.repos.yaml workspace_id=aiworks (~/aiworks).
# OMP launches with cwd here so the UI displays the configured workspace, not
...
# Expose nested OMP project configs (.claude from UltimateCoders, .omp from
# vendor/oh-my-pi) up into OMP_WORKSPACE so single-cwd discovery finds them.
setup_omp_workspace() {
