...
def test_real_corpus_reproduces_the_recorded_numbers():
    """Real corpus as of T38 / #688: 793 references / 0 dangling / 0 malformed
    (T38's own jsonl then moves it to 794 -- see the T38 note below).
    ...
    # counts only once committed: 793 while it is untracked, 794 once it is
    ...
    assert len(refs) in (793, 794), f"reference count drifted: {len(refs)}"
