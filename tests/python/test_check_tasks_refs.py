...
def test_real_corpus_reproduces_the_recorded_numbers():
    """Real corpus as of T37 / #687: 792 references / 0 dangling / 0 malformed
    (T37's own jsonl then moves it to 793 -- see the T37 note below).
    ...
    # counts only once committed: 792 while it is untracked, 793 once it is
    ...
    assert len(refs) in (792, 793), f"reference count drifted: {len(refs)}"
