"""Unit tests for CodegraphClient and Worker codegraph integration."""
...
class TestWorkerCodegraphIntegration:
    """Tests for Worker codegraph tool integration."""

    def test_worker_has_codegraph_tools(self):
        """Worker should have codegraph tools in its tool registry."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        assert "symbol_search" in worker.tools
        assert "find_callers" in worker.tools
        assert "find_callees" in worker.tools
        assert "impact_analysis" in worker.tools
        assert "explore_code" in worker.tools

    def test_worker_has_codegraph_tool_definitions(self):
        """Worker should have codegraph tool definitions."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        names = [t.name for t in worker._tool_definitions]
        assert "symbol_search" in names
        assert "find_callers" in names
        assert "find_callees" in names
        assert "impact_analysis" in names
        assert "explore_code" in names

    @pytest.mark.asyncio
    async def test_symbol_search_tool_when_unavailable(self):
        """symbol_search should return error when codegraph is unavailable."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        result = await worker._tool_symbol_search(query="test")
        parsed = json.loads(result)
        assert "error" in parsed
        assert "not available" in parsed["error"].lower()

    @pytest.mark.asyncio
    async def test_find_callers_tool_when_unavailable(self):
        """find_callers should return error when codegraph is unavailable."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        result = await worker._tool_find_callers(symbol="test")
        parsed = json.loads(result)
        assert "error" in parsed

    @pytest.mark.asyncio
    async def test_find_callees_tool_when_unavailable(self):
        """find_callees should return error when codegraph is unavailable."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        result = await worker._tool_find_callees(symbol="test")
        parsed = json.loads(result)
        assert "error" in parsed

    @pytest.mark.asyncio
    async def test_impact_analysis_tool_when_unavailable(self):
        """impact_analysis should return error when codegraph is unavailable."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        result = await worker._tool_impact_analysis(symbol="test")
        parsed = json.loads(result)
        assert "error" in parsed

    @pytest.mark.asyncio
    async def test_explore_code_tool_when_unavailable(self):
        """explore_code should return error message when codegraph is unavailable."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        result = await worker._tool_explore_code(query="test")
        assert "not available" in result.lower()

    @pytest.mark.asyncio
    async def test_prior_context_includes_codegraph(self):
        """_gather_prior_context should include codegraph context when available."""
        # Create a worker with a mock codegraph
        worker = Worker(worker_id="w1", execution_mode="llm")
        # Replace codegraph with a mock
        mock_cg = MagicMock()
        mock_cg.is_available.return_value = True
        mock_cg.explore.return_value = "## Code Knowledge Graph\n- **test_func** (function)"
        worker._codegraph = mock_cg

        subtask = Subtask(id="s1", parent_id="t1", description="test subtask")
        context = await worker._gather_prior_context(subtask)

        assert "Code Knowledge Graph" in context
        mock_cg.explore.assert_called_once_with("test subtask", max_nodes=10)

    @pytest.mark.asyncio
    async def test_prior_context_without_codegraph(self):
        """_gather_prior_context should work when codegraph is unavailable."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        # Default CodegraphClient with empty path -> unavailable
        subtask = Subtask(id="s1", parent_id="t1", description="test subtask")
        context = await worker._gather_prior_context(subtask)
        # Should not contain codegraph section, just "(no prior context)"
        assert "Code Knowledge Graph" not in context

    @pytest.mark.asyncio
    async def test_prior_context_codegraph_error_graceful(self):
        """_gather_prior_context should handle codegraph errors gracefully."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        mock_cg = MagicMock()
        mock_cg.is_available.return_value = True
        mock_cg.explore.side_effect = RuntimeError("DB error")
        worker._codegraph = mock_cg
    ...
    @pytest.mark.asyncio
    async def test_sandbox_prompt_includes_codegraph(self):
        """Sandbox prompt should include codegraph context when available."""
        worker = Worker(worker_id="w1", execution_mode="sandbox")
        # Replace codegraph with a mock
        mock_cg = MagicMock()
        mock_cg.is_available.return_value = True
        mock_cg.explore.return_value = "## Code Knowledge Graph\n- **test_func** (function)"
        ...
        # Patch SandboxManager.execute to avoid actually running it
        mock_output = AgentOutput(summary="done", file_changes=[], success=True)
        worker._sandbox_manager = MagicMock()
        worker._sandbox_manager.execute = AsyncMock(return_value=mock_output)

        subtask = Subtask(id="s1", parent_id="t1", description="Implement feature X")
        result = await worker._execute_in_sandbox(subtask)

        # Verify sandbox was called and codegraph explore was used
        assert result.success
        mock_cg.explore.assert_called_once_with("Implement feature X", max_nodes=10)
        # Verify the prompt included codegraph context
        call_args = worker._sandbox_manager.execute.call_args
        prompt = call_args[0][0]
        assert "Code Knowledge Graph" in prompt

    @pytest.mark.asyncio
    async def test_sandbox_prompt_without_codegraph(self):
        """Sandbox prompt should work without codegraph context."""
        worker = Worker(worker_id="w1", execution_mode="sandbox")
        # Default CodegraphClient with empty path -> unavailable
        ...
        assert result.success
        # Verify the prompt used fallback context
        call_args = worker._sandbox_manager.execute.call_args
        prompt = call_args[0][0]
        assert "sandbox mode: prior context not gathered" in prompt

    @pytest.mark.asyncio
    async def test_symbol_search_tool_with_mock_codegraph(self):
        """symbol_search tool should delegate to CodegraphClient.search."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        mock_cg = MagicMock()
        mock_cg.is_available.return_value = True
        mock_cg.search.return_value = [
            {"name": "test_func", "kind": "function", "file_path": "test.py", "start_line": 1}
        ]
        worker._codegraph = mock_cg

        result = await worker._tool_symbol_search(query="test_func")
        parsed = json.loads(result)
        assert len(parsed) >= 1
        assert parsed[0]["name"] == "test_func"

    @pytest.mark.asyncio
    async def test_impact_analysis_tool_with_mock_codegraph(self):
        """impact_analysis tool should return structured results."""
        worker = Worker(worker_id="w1", execution_mode="llm")
        mock_cg = MagicMock()
        mock_cg.is_available.return_value = True
        mock_cg.impact.return_value = [
            {"name": "caller_func", "kind": "function", "file_path": "test.py", "start_line": 10}
        ]
        worker._codegraph = mock_cg

        result = await worker._tool_impact_analysis(symbol="test_func")
        parsed = json.loads(result)
        assert parsed["symbol"] == "test_func"
        assert parsed["affected_count"] == 1
